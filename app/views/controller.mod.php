...
class <?php echo $controller ?> extends Controller {
	private $_model, $_uh, $_redirectBase;
	...
	function save() {
		...
		if($this->_model->saveData($this->Request->Post->value(), $is_edit)) {
			...
			$this->redirect($this->_redirectBase . '/index/' . $page_limit . '/' . $page, 'Sukses menyimpan data');
		}
		else $this->redirect($this->_redirectBase . '/index', 'Gagal ketika menyimpan data');
	}
